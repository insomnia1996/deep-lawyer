...
#tstr=" 公诉 机关 指控 被告人 李某 于 2012年 12月 1日 19时 许 ， 到 广州市 南沙区 新 垦 十四 涌 西面 机耕路 附近 的 薯丛 中 ， 用 随身 携带 的 摩托车 钥匙 将 被害人 赵某 的 两 轮 摩托车 （ 发动机 号 ： GP 100104121 ， 车架号 ： LCS5BPK H 458762310 ， 价 值 人民币 3318 元 ） 的 车头 锁 打开 后 盗走 ， 当 被告人 李某 将 该 摩托车 推 到 路边 准备 打火 启动 时 被 公安人员 人赃 并 获 ， 被告人 李某 归案 后 向 公安 机关 如实 供述 自己 的 犯罪 事实 。 案 发 后 ， 公安 机关 将 涉案 摩托车 发 还 被害人 赵某 。 公诉 机关 建议 本院 对 被告人 李某 在 ×× 至 一 年 的 幅度 内 量刑 ， 并 处 罚金 ， 若 对 被告人 李某 落实 监管 措施 ， 可 适用 ×× 。"
...
jsonlst=[]
def readjson():
	f=open('data/basic/new_bigdata_test_cuted.json','r',encoding='utf-8')
	
	while True:
		line=f.readline()
		if line:
			try:
				jsdic=json.loads(line)
				#print(jsdic)
				tstr=jsdic['fact_cut']
				jsonlst.append(tstr)
			except:
...
def thread_con(i):
	#g=open('data/basic/big_data_train_pair%d.json' %i,'a')
	g=open('data/basic/tst%d.json' %i,'a')
	for j in range(i,len(jsonlst),15):
		ttstr=jsonlst[j]
		sentencelst=re.split('，|；|。',ttstr)[:-1]
		try:
			linelst=[]
			writedic={}
			writedic['num']=j
			if len(sentencelst)>64:
				sentencelst=sentencelst[:64]
			for tstr in sentencelst:			
				tmplst=tstr.split()
				if len(tmplst)>64:
					tstr=''
					for k in tmplst[:64]:
				...
				for triple in dep.triples():
					if (triple[0][1] in nlist and triple[2][1] in vlist) or (triple[0][1] in vlist and triple[2][1] in nlist) or (triple[0][1] in nlist and triple[2][1] in nlist):
						#print(triple)
						data={}
						data['former']=triple[0][0]
						data['latter']=triple[2][0]
						data['rel']=triple[1]
						linelst.append(data)
			writedic['pair']=linelst
			gstr=json.dumps(writedic)
			g.write(gstr+'\n')
		except:
			print('error on thread%d on line %d' %(i,j))
...
if __name__=='__main__':
	readjson()
	for m in range(15):
		sl=threading.Thread(target=thread_con,args=(m,))
		sl.start()
	#thread_con(1)
